...
def solve1(input):
    vm = VM(convert(input), [], name = 'Day 13.1')
    state = 0
    tiles = {}
    ...
    while vm.running:
        ...
        if vm.state == 'output':
            if state == 0:
                ...
                tiles[(x,y)] = vm.last_output
                print(f'{x},{y} {vm.last_output}')
                state = 0
        elif vm.state == 'input':
            print('input')
        ...
        if vm.state == 'output':
            if state == 0:
                ...
                if x == -1 and y == 0:
                    ...
                    #if game_on:
                        #system('clear')                  
                        #show_table(tiles, score, max_x, max_y)
            ...
            game_on = True
            if paddleY != None and ball != None:
                if paddleY < ball[0]:
                    vm.inputs.append(1)
                    if verbose: print('going 1')
                if paddleY > ball[0]:
                    vm.inputs.append(-1)
                    if verbose: print('going -1')
                if paddleY == ball[0]:
                    vm.inputs.append(0)
                    if verbose: print('staying')

            if verbose: system('clear')                  
            if verbose: show_table(tiles, score, max_x, max_y)

    print('Score: ', score)
def show_table(tiles, score, max_x, max_y):
    tile_map = {
        0: ' ',
        1: 'W',
        2: '#',
...
class VM():
    ...
    def run(self):
        ...
        if state == 'output':
            ...
            self.state = 'input'
            if verbose: print(f'{self.name} paused')
